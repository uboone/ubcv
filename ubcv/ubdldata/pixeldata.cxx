#ifndef PIXELDATA_CXX
#define PIXELDATA_CXX
...
namespace ubdldata {
      ...
      _nrows(nrows)
  {
    ...
    if ( _ncols<=0 ) throw std::runtime_error( "pixeldata::pixeldata(...) ncols<=0!" );
    if ( _nrows<=0 ) throw std::runtime_error( "pixeldata::pixeldata(...) nrows<=0!" );
    if ( _dim_per_point<2 ) throw std::runtime_error( "pixeldata::pixeldata(...) dim_per_point needs to be >=2" );
      ...
      if ( !data_is_array_indices ) {
        ...
        for ( size_t ipt=0; ipt<data_v.size(); ipt++ ) {
          ...
          if ( (int)ptdata.size()>dim_per_point )
            throw std::runtime_error( "pixeldata::pixeldata(...) dimension of point is > dim_per_point" );
    ...
    }//end of if number of points>0
    else {
      // empty, so pass empty bounding box
      _bbox_v = { 0, 0, 0, 0 };
    }
  ...
  float pixeldata::pos_x( int col, bool checkbounds ) const {
    float x = _meta_xmin + _pixel_width*col;
    if ( checkbounds && ( col<0 || col>=cols() || x<_meta_xmin || x>=_meta_xmax ) ) throw std::runtime_error("pixeldata::pos_x(col) out of bounds");
    return x;
  }

  float pixeldata::pos_y( int row, bool checkbounds ) const {
    float y = _meta_ymin + _pixel_height*row;
    if ( checkbounds && ( row<0 || row>=rows() || y<_meta_ymin || y>=_meta_xmax ) ) throw std::runtime_error("pixeldata::pos_y(row) out of bounds");
    return y;
  }
  
  int pixeldata::col( float x, bool checkbounds ) const {
    int c = (x-_meta_xmin)/_pixel_width;
    if ( checkbounds && ( x<_meta_xmin || x>=_meta_xmax || c<0 || c>=cols() ) ) throw std::runtime_error("pixeldata::col(x) out of bounds");
    return c;
  }

  int pixeldata::row( float y, bool checkbounds ) const {
    int r = (y-_meta_ymin)/_pixel_height;
    if ( checkbounds && ( y<_meta_ymin || y>=_meta_ymax || r<0 || r>=rows() ) ) throw std::runtime_error("pixeldata::row(y) out of bounds");
    return r;
  }
  
  bool pixeldata::contains( float x, float y ) const {
    if ( x<_meta_xmin || x>=_meta_xmax || y<_meta_ymin || y>=_meta_ymax ) return false;
    return true;
  }

  bool pixeldata::contains_index( int col, int row ) const { 
    if ( col<0 || col>=cols() || row<0 || row>=rows() )  return false;
    return true;
  }

  std::vector<float> pixeldata::point( int index ) const {
    std::vector<float> out( _data_v.begin()+index*_dim_per_point, _data_v.begin()+(index+1)*_dim_per_point );
    return out;
  }
  
  std::vector<int>   pixeldata::pixelpos( int index ) const {
    std::vector<float> out( _data_v.begin()+index*_dim_per_point, _data_v.begin()+(index+1)*_dim_per_point );
  ...
  std::vector<float> pixeldata::as_vector_mask( float meta_xmin, float meta_ymin,
  ...
  std::vector<float> pixeldata::as_vector_mask() const {
    // provides an unrolled (row,col) vector which represents the mask points densly
  ...
  std::vector<float> pixeldata::as_vector_bbox() const { return _bbox_v; }

  std::vector<float> pixeldata::as_vector_bbox( float meta_xmin, float meta_ymin,
  ...
  std::vector<int> pixeldata::as_vector_array_bounds( float meta_xmin, float meta_ymin, float meta_xmax, float meta_ymax ) const {
  ...
  std::vector<int> pixeldata::as_vector_array_bounds() const {
    return as_vector_array_bounds( _meta_xmin, _meta_ymin, _meta_xmax, _meta_ymax );
  }
    

}//end of namespace ubdldata
#endif
