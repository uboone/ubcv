...
files = {"orig":"larcv_segment_origtest.root",
         "reduced":"larcv_segment_compressed.root"}
...
fout = rt.TFile("output_compare_segment.root","recreate")
...
for ientry in range(runN):
    ...
    for ftype in types:
        ...
        ev_segment_v[ftype] = iomans[ftype].get_data( larcv.kProductImage2D, "segment" )
    ...
    if NROWS==1:        
        entry_canvas.Draw()
        entry_canvas.SaveAs("compare_segment_entry%02d.png"%(ientry))
        entry_canvas.Write()
        break
    ...
    for iimg in range(NROWS-1):
        horig    = rt.TH2D("horig_%02d"%(iimg),"segment-original[%d]"%(iimg),3456,0,3456,1008,0,1008)
        hreduced = rt.TH2D("hreduced_%02d"%(iimg),"segment-reduced[%d]"%(iimg),3456,0,3456,1008,0,1008)
        hdiff    = rt.TH2D("hdiff_%02d"%(iimg),"diff[%d]"%(iimg),3456,0,3456,1008,0,1008)
        
        orig    = ev_segment_v["orig"].at(iimg)
        reduced = ev_segment_v["reduced"].at(iimg)
